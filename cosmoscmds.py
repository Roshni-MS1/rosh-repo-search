...
def create_items(container):
    ...
    label_def= get_new_Label("Confidential")
    container.create_item(body=label_def)
    label_def= get_new_Label("Public")
    container.create_item(body=label_def)
...
def get_new_Label(item_id):
    # create a new label
    label = { 
                'id' : item_id,
                'partitionKey' : item_id,
                'create_date' : datetime.date(2005,7,11).strftime('%c'),
            }
...
def run_sample():
    ...
    try:
        ...
        scale_container(container)
        create_items(container)
        #read_item(container, 'Confidential', 'Confidential')
        read_items(container)
        query_items(container, 'Confidential')
        #replace_item(container, 'SalesOrder1', 'Account1')
        #upsert_item(container, 'SalesOrder1', 'Account1')
        #delete_item(container, 'SalesOrder1', 'Account1')
        delete_container()
